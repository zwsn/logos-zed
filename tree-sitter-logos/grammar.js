...
function logosLineDirective(prefix) {
  return token(seq(prefix, /[^\n]*/));
}
...
module.exports = grammar(C, {
    ...
    original
      .filter(
        (e) =>
          !(
            e.length == 2 &&
            e[0].name == "type_qualifier" &&
            e[1].name == "extension_expression"
          ),
      )
      .concat([
        [$.enum_specifier],
  ...
  supertypes: ($, original) => original.concat([$.specifier_qualifier]),
  ...
  rules: {
    ...
    _non_case_statement: ($, original) =>
      choice(
        ...original.members.filter(
          (member) =>
            member.name !== "seh_try_statement" &&
            member.name !== "seh_leave_statement",
        ),
        $.try_statement,
        $.throw_statement,
    ...
    logos_hook_block: ($) =>
      seq(
        field("directive", $.logos_hook_directive_line),
        repeat($.logos_hook_member),
        field("end", $.logos_end_directive),
      ),

    logos_group_block: ($) =>
      seq(
        field("directive", $.logos_group_directive_line),
        repeat($.logos_group_member),
        field("end", $.logos_end_directive),
      ),

    logos_subclass_block: ($) =>
      seq(
        field("directive", $.logos_subclass_directive_line),
        repeat($.logos_subclass_member),
        field("end", $.logos_end_directive),
      ),
    ...
    logos_hook_directive_line: (_) => token(seq("%hook", /[ \t]+[^\n]*/)),
    logos_group_directive_line: (_) => token(seq("%group", /[ \t]+[^\n]*/)),
    logos_subclass_directive_line: (_) =>
      token(seq("%subclass", /[ \t]+[^\n]*/)),

    logos_property_directive_line: (_) =>
      choice(logosLineDirective("%property"), logosLineDirective("%new")),

    logos_top_level_directive_line: (_) =>
      choice(logosLineDirective("%config"), logosLineDirective("%hookf")),

    logos_ctor_definition: ($) =>
      seq(
        field("directive", $.logos_ctor_directive),
        field("body", $.compound_statement),
      ),

    logos_dtor_definition: ($) =>
      seq(
        field("directive", $.logos_dtor_directive),
        field("body", $.compound_statement),
      ),

    logos_ctor_directive: (_) => "%ctor",
    logos_dtor_directive: (_) => "%dtor",
    logos_end_directive: (_) => "%end",

    logos_inline_directive: (_) =>
      choice(
        token(seq("%init", /[^;\n]*/)),
        token(seq("%c", choice("(", " "), /[^;\n]*/)),
        token(seq("%orig", /[^;\n]*/)),
          ...
          commaSep(
            choice(
              seq(
                ...
                choice($.expression, $.typeof_specifier),
              ),
              $.compound_statement,
            ),
          ),
    ...
    compound_statement: ($) =>
      seq(optional("@autoreleasepool"), "{", repeat($._block_item), "}"),
    ...
    availability: ($) =>
      seq(
        ...
        commaSep1(
          seq(
            $.identifier,
            optional(seq("=", choice($.version, $.expression))),
          ),
        ),
        ")",
      ),
    ...
    _declarator: ($, original) =>
      prec.right(
        choice(
          ...original.members.filter(
            (member) => member.name !== "attributed_declarator",
          ),
    ...
    _abstract_declarator: ($, original) =>
      choice(original, $.abstract_block_pointer_declarator),
    ...
    preproc_include: ($) =>
      seq(
        field(
          "directive",
          choice(preprocessor("include"), preprocessor("import")),
        ),
        field(
          "path",
          choice(
    ...
    ...preprocIf("", ($) =>
      choice($._block_item, $.attribute_specifier, $.property_implementation),
    ),
    ...preprocIf(
      "_in_implementation_definition",
      ($) => $.implementation_definition,
    ),
    ...
    _preproc_expression: ($, original) => choice(original, $.system_lib_string),
    ...
    attribute_specifier: ($) =>
      seq(
        ...
        choice(
          $.argument_list,
          seq("(", commaSep1(choice("noreturn", "nothrow")), ")"),
        ),
        ")",
      ),
          ...
          seq(
            ...
            optional(
              seq(
                ...
                field(
                  "underlying_type",
                  choice($._type_identifier, $.primitive_type),
                ),
              ),
            ),
    ...
    generic_arguments: ($) =>
      prec.right(seq("(", commaSep1($._type_identifier), ")")),
    ...
    visibility_specification: (_) =>
      choice("@private", "@protected", "@package", "@public"),
      ...
      seq(
        ...
        optional(
          seq(
            ...
            choice(
              "...",
              commaSep1(alias($.c_method_parameter, $.method_parameter)),
            ),
          ),
        ),
        repeat($.declaration),
    ...
    method_selector: ($) =>
      prec.left(
        choice($.method_selector_no_list, seq($.keyword_selector, ",")),
      ),
    ...
    keyword_declarator: ($) =>
      seq(optional($.identifier), ";", optional($.method_type), $.identifier),
    ...
    property_attributes_declaration: ($) =>
      seq("(", commaSep($.property_attribute), ")"),

    property_attribute: ($) =>
      choice($.identifier, seq($.identifier, "=", $.identifier, optional(":"))),
    ...
    atomic_declaration: ($) =>
      seq("_Atomic", "(", $.type_specifier, ")", $._field_identifier, ";"),
    ...
    specifier_qualifier: ($) =>
      prec.right(
        choice($.type_specifier, $.type_qualifier, $.protocol_qualifier),
      ),

    struct_declarator: ($) =>
      choice($._declarator, seq(optional($._declarator), ":", $.expression)),
    ...
    finally_clause: ($) =>
      seq(choice("@finally", "__finally"), $.compound_statement),
    ...
    available_expression: ($) =>
      seq(
        ...
        "(",
        commaSep1(choice($.identifier, seq($.identifier, $.version), "*")),
        ")",
      ),
        ...
        seq(
          ...
          repeat1(
            seq(
              ...
              repeat(seq(":", commaSep1($.expression))),
            ),
          ),
          "]",
        ),
    ...
    va_arg_expression: ($) =>
      seq("va_arg", "(", $.expression, ",", $.type_descriptor, ")"),

    ms_asm_block: (_) => seq("__asm", "{", /[^}]*/, "}"),
    ...
    abstract_parenthesized_declarator: ($) =>
      prec(
        1,
        seq("(", repeat($._declaration_modifiers), $._abstract_declarator, ")"),
      ),
    ...
    generic_specifier: ($) =>
      prec.right(
        seq($._type_identifier, repeat1(seq("<", commaSep1($.type_name), ">"))),
      ),

    type_specifier: ($, original) =>
      choice(
        original,
        $.typedefed_specifier,
        $.generic_specifier,
    ...
    typedefed_identifier: (_) => choice("BOOL", "IMP", "SEL", "Class", "id"),

    typedefed_specifier: ($) =>
      prec.right(
        seq($.typedefed_identifier, optional($.protocol_reference_list)),
      ),
    ...
    protocol_qualifier: (_) =>
      choice("out", "inout", "bycopy", "byref", "oneway", "in"),
    ...
    at_expression: ($) => prec.right(seq("@", $.expression)),
    ...
    dictionary_literal: ($) =>
      seq(
        "@",
        "{",
        optional(seq(commaSep1($.dictionary_pair), optional(","))),
        "}",
      ),
    ...
    keyword_identifier: ($) =>
      alias(prec(-3, choice("id", "in", "struct", "const")), $.identifier),
  },
});
...
function preprocIf(suffix, content) {
  ...
  return {
    ...
    ["preproc_else" + suffix]: ($) =>
      seq(preprocessor("else"), repeat(prec(2, content($)))),
